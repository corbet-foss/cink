...
const consumer = mkdtempSync(join(tmpdir(), 'cink-consumer-'));
...
if (installedMetadata.license !== pkg.license) throw new Error('Installed package license differs');
const licenseRoot = resolve('../../../LICENSES');
const licenseNames = readdirSync(licenseRoot).sort();
if (JSON.stringify(readdirSync(join(installed, 'LICENSES')).sort()) !== JSON.stringify(licenseNames)) {
    throw new Error('Installed license inventory differs');
}
for (const name of licenseNames) {
    if (!readFileSync(join(licenseRoot, name)).equals(readFileSync(join(installed, 'LICENSES', name)))) {
        throw new Error(`Installed license text differs: ${name}`);
    }
}
